...
public class DBHelper extends SQLiteOpenHelper {
	...
	// 数据库文件目标存放路径为系统默认位置，com.huabao.flipped 是你的包名
	//private static String DB_PATH = "/data/data/com.huabao.flipped/databases/";
	/*
	...
	public void createDataBase(final File parent) throws IOException {
		boolean dbExist = checkDataBase(parent);
		if (dbExist) {
			// 数据库已存在，do nothing.
		} else {
			// 创建数据库
			try {
				File dbf = new File(parent, DB_NAME);
				if (dbf.exists()) {
					dbf.delete();
				}
	...
	// 检查数据库是否有效
	private boolean checkDataBase(final File parent) {
		SQLiteDatabase checkDB = null;
		String path = parent.getAbsolutePath() + "/" + DB_NAME;
		try {
